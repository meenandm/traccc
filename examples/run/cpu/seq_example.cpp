...
int seq_run(const traccc::opts::input_data& input_opts,
    ...
    for (std::size_t event = input_opts.skip;
	    ...
	    const auto& all_clusters = clusters_per_event.cell_indices();
	    
            std::cout << "Number of clusters: " << all_clusters.size() << std::endl;
            for (size_t cluster_idx = 0; cluster_idx < all_clusters.size(); ++cluster_idx) {
                 const auto& cell_indices = all_clusters[cluster_idx];
                 std::cout << "Cluster " << cluster_idx << " contains cells: ";
                 for (const auto& cell_idx : cell_indices) {
                     std::cout << cell_idx << " ";
                 }
             std::cout << std::endl;
             }
